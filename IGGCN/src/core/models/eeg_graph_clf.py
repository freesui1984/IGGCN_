...
from ..layers.GGNNmodel import GCRNNModel_classification, GCRNNModel_nextTimePred
...
class EEGGraphClf(nn.Module):
    def __init__(self, config):
        ...
        gcgru_activation = config['gcgru_activation']
    ...
    def prepare_init_graph(self, context, context_lens, adj):
        raw_context_vec = context.reshape(len(context_lens), self.config['channel'],
                                          self.config['eeg_embed_dim'])
        # raw_context_vec = context.reshape(len(context_lens), self.config['min_word_freq'], self.config['channel'], self.config['eeg_embed_dim'])
        raw_context_vec = dropout(raw_context_vec, self.word_dropout, shared_axes=[-2], training=self.training)
        context_vec = self.ctx_rnn_encoder(raw_context_vec, context_lens)[0].transpose(0, 1)
        init_adj = adj.reshape(len(context_lens), self.config['channel'], self.config['channel'])
        return raw_context_vec, context_vec, init_adj
    def reshape(self, context, context_lens, adj):
        raw_context_vec = dropout(context, self.word_dropout, shared_axes=[-2], training=self.training)
        context_vec = self.fc(context)
        init_adj = adj.reshape(len(context_lens), self.config['channel'], self.config['channel'])
        return raw_context_vec, context_vec, init_adj
