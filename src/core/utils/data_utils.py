...
# 准备数据操作（所有数据集通用的，会根据config选择如何准备）
def prepare_datasets(config):
    data = {}
    # 网络结构的数据
    if config['data_type'] == 'network':
        adj, features, labels, idx_train, idx_val, idx_test = network_data_utils.load_data(
            config['data_dir'], config['dataset_name'], knn_size=config.get('input_graph_knn_size', None),
            epsilon=config.get('input_graph_epsilon', None), knn_metric=config.get('knn_metric', 'cosine'),
            prob_del_edge=config.get('prob_del_edge', None), prob_add_edge=config.get('prob_add_edge', None),
            seed=config.get('data_seed', config['seed']), sparse_init_adj=config.get('sparse_init_adj', False))
...
class EEGDataStream(object):
    # all_instances:text传入的是train_set
