...
from ..layers.gnn import EEGGraphAttentionLayer
...
class GraphLearner(nn.Module):
    # 初始化
    def __init__(self, config, input_size, hidden_size, topk=None, epsilon=None, num_pers=16, metric_type='attention', device=None):
        ...
        self.alpha = config.get('gat_alpha', 0.2)

        # 选择图拓扑的计算依据
        if metric_type == 'attention':
            # num_pers: graph_learn_num_pers  多头注意力机制的头数
            # 一共有num_pers个全连接层
            ...
            # graph_learn_num_pers:多头注意力机制
            self.weight_tensor = torch.Tensor(num_pers, input_size)
            # xavier初始化：基本思想是通过网络层时，输入和输出的方差相同，包括前向传播和后向传播
            # 用nn.Parameter()来将这个随机初始化的Tensor注册为可学习的参数Parameter
        ...
        elif metric_type == 'EEGGraph_attention':
            self.linear_sims1 = nn.ModuleList([nn.Linear(input_size, 1, bias=False) for _ in range(num_pers)])
            self.linear_sims2 = nn.ModuleList([nn.Linear(input_size, 1, bias=False) for _ in range(num_pers)])
            ...
            # weighted_cosine 和 EEGGraph_attention 组合
            # graph_learn_num_pers:多头注意力机制
            self.weight_tensor = torch.Tensor(num_pers, input_size)
            # xavier初始化：基本思想是通过网络层时，输入和输出的方差相同，包括前向传播和后向传播
            # 用nn.Parameter()来将这个随机初始化的Tensor注册为可学习的参数Parameter
    ...
    def forward(self, context, adj, ctx_mask=None):
        ...
        if self.metric_type == 'attention':
            ...
            # torch.matmul:内乘，当输入超过2维时，把多出的一维作为batch提出来，其他部分做矩阵乘法。
            # attention的维度是  （边*边）
        ...
        elif self.metric_type == 'EEGGraph_attention':
            attention_head = []
            attention = []
            # 多头
            ...
            # torch.matmul:内乘，当输入超过2维时，把多出的一维作为batch提出来，其他部分做矩阵乘法。
            # attention的维度是  （边*边）
            attention_cos = torch.matmul(context_norm, context_norm.transpose(-1, -2)).mean(0)
            markoff_value = 0
            ...
            for _ in range(len(self.linear_sims1)):
                a_input1 = self.linear_sims1[_](context)
                a_input2 = self.linear_sims2[_](context)
                attention_eeg.append(self.leakyrelu(a_input1 + a_input2.transpose(-1, -2)))

            attention_eeg = torch.mean(torch.stack(attention_eeg, 0), 0)
            attention = (attention_cos + attention_eeg)/2
            markoff_value = -INF
